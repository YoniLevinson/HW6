...
class generic_packet {
     ...
     * @param start_index The index (0-based) of the delimiter after which extraction begins.
     *                    if 0 start from the beginning of the string.
     * @param end_index The index (0-based) of the delimiter before which extraction ends.
    ...
    /* Validate end_index. */
    if (end_index <= start_index || end_index-1 > static_cast<int>(positions.size())) {
        return "";
    }

    size_t len = positions[end_index-1] - start;
